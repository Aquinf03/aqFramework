...
- Unsupported: `formats` (errors, not faked). `objective: mtp` needs `n_predict >= 2`. `paged_kv` is recorded at train; serve is not paged yet. `speculative: true` needs `draft_model:` — serve uses HF assisted generation.
