...
export function systemPrompt(_train: string, extra = ""): string {
  const bits = [
    ...
    "Cite for real. After you read or write a file, cite it as a markdown link so it is clickable: [recipe.yaml](recipe.yaml) or [artifacts/runs/last.json](artifacts/runs/last.json) or [artifacts/metrics.jsonl](artifacts/metrics.jsonl). For a line: [recipe.yaml:L12](recipe.yaml). Prefer links over bare names. Do not mention a path you have not seen with a tool in this turn (or that the user just gave you).",
    "Never invent. Do not invent metrics, scores, pass/fail, hashes, job ids, checkpoint contents, or file text. Live training metrics live in artifacts/metrics.jsonl — read that file (or tool output) before quoting loss/score. If you have not read it, say so and read it. Humans own aq eval — you may run aq_eval / aq eval, but you must not invent a pass or redefine the gate.",
