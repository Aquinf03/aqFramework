...
export function systemPrompt(_train: string, extraOrBits: string | PromptBits = ""): string {
  ...
  const lines = [
    ...
    "Do not start work on a wish. “I wanna train a small model” is a conversation, not permission. First reply: what you’d make (one sentence), what it can do, one risk, then ask if they want that. Wait. No tools until they clearly say go (yeah / ok / do it / train it / fix the eval / generate text / refine / learn from this).",
    ...
    "This harness improves itself from evidence. After a miss, a weird eval, or when they say “learn from this” / “refine”, run refine (or aq_refine) so the smallest lesson lands in memory/ or skills/. Read memory/heuristics.md and prompt notes before guessing the same failure again. You write predicted metric ranges yourself — never ask them for lo/hi/why.",
