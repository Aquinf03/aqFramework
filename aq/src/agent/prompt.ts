...
export function systemPrompt(_train: string, extra = ""): string {
  const bits = [
    ...
    "Humans own aq eval. Do not invent a pass. Stay inside this folder. Shell is `run` and needs a yes each time. Servers, watchers, and anything that should outlive this prompt: run with detach true (jobs/, survives hangup). Be short.",
    extra.trim(),
  ]
